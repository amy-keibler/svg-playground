...
spec = do
  describe "linearAnimate" $ do
    it "should animate directly to end in two frames" $ do
      linearAnimate 2 (Point 0 1) (Point 1 0) `shouldBe` [(Point 0 1), (Point 1 0)]
    it "should animate in-between the points once for three frames" $ do
      linearAnimate 3 (Point 0 2) (Point 2 0) `shouldBe` [(Point 0 2), (Point 1 1), (Point 2 0)]
  describe "iterate" $ do
    it "should iterate between zero and 2" $ do
      interpolate 3 0 2 `shouldBe` [0, 1, 2]
    it "should iterate between 10 and 50" $ do
      interpolate 9 10 50 `shouldBe` [10, 15, 20, 25, 30, 35, 40, 45, 50]
  describe "linearAnimateLine" $ do
    it "should interpolate the line over 2" $ do
